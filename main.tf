locals {
  ...
  kafka_listeners = length(var.kafka_listeners) > 0 ? var.kafka_listeners : (var.kafka_listener_type == "secure" ? local.kafka_listeners_secure : local.kafka_listeners_insecure)
  values_content = {
